...
import com.samsoftx.sample.config.KafkaProperties;
...
class JhSampleApplicationKafkaResourceIT {
    ...
    void setup() {
        ...
        JhSampleApplicationKafkaResource kafkaResource = new JhSampleApplicationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jh-sample-application-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jh-sample-application-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
